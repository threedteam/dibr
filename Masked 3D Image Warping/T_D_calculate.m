...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [T_D] = T_D_calculate(rx_iref, rx_d, rx_min_z, rx_max_z, rx_pr, rx_ides_rt, rx_ps)
        ...
        case {'F'}
            ...
            for i = Rect(2) : Rect(4)  % 按行扫描
                for j = Rect(1) : Rect(3)
                    ...
                    if i < Rect(4) && j < Rect(3)
                        d_1 = rx_d(i + 1, j + 2);   %旁边一个点在深度图中的深度值
                        z_1 = DepthLevelToZ(d_1, rx_min_z, rx_max_z);  % 旁边一个点的世界坐标的值z
                        [x_1, y_1] = projUVZtoXY(m_Height, rx_pr, double(j + 1), double(i), z_1);
                        [u1_double, v1_double] = projXYZtoUV(m_Height, rx_ps, x_1, y_1, z_1);
                        u1_round = round(u1_double); v1_round = round(v1_double);
                        sub_u = u1_round - u_round;
                        if (abs(sub_u) >= 3)
                            %fprintf('i = %d, j = %d\n', i + 1, j + 1);
                            % Set the pixel color to red in tx_ides
                            D_uv = int32(rx_d(i + 1, j + 1)) - int32(rx_d(i + 1, j + 2));
                            if (D_uv > 0)
                                rx_iref(i + 1, j + 1, :) = [255, 0, 0];
                                rx_iref(i + 1, j + 2, :) = [0, 0, 255];
                            end
                            if (D_uv < 0)
                                rx_iref(i + 1, j + 1, :) = [0, 0, 255];
                                rx_iref(i + 1, j + 2, :) = [255, 0, 0];
                            end
                            
                            %fprintf('D__uv = %d\n', abs(D_uv));
                            T_D = min(abs(D_uv), T_D);
                        end
                    end
